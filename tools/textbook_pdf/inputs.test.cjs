...
function fixture(t) {
  ...
  for(let i=1;i<=35;i++)fs.writeFileSync(path.join(dir,`${String(i).padStart(2,'0')}_chapter.md`),'# Chapter');
  return {root,dir};
}
test('filesystem discovery accepts a complete candidate without Git history',t=>{
  const {root,dir}=fixture(t); assert.equal(discoverChapters(root).length,35);
  fs.unlinkSync(path.join(dir,'13_chapter.md'));assert.throws(()=>discoverChapters(root),/35|Missing/);
});
