...
canonical_project_dir=$(CDPATH= cd -- "$script_dir/../formalization/nng4_intro" && pwd)
...
if ! command -v lake >/dev/null 2>&1; then
  printf '%s\n' "NNG4 introductory Lean toolchain is unavailable" >&2
  exit 1
fi

if [ "$normal_build" = true ]; then
  if [ -z "${ELAN_HOME:-}" ]; then
    printf '%s\n' "NNG4 introductory Lean verification requires a task-scoped ELAN_HOME" >&2
    exit 1
  fi
  if ! elan_home_dir=$(CDPATH= cd -P -- "$ELAN_HOME" && pwd -P); then
    printf '%s\n' "NNG4 introductory Lean verification requires a canonical task-scoped ELAN_HOME directory" >&2
    exit 1
  fi
  ...
  if ! IFS= read -r pinned_toolchain < "$canonical_project_dir/lean-toolchain" || [ -z "$pinned_toolchain" ]; then
    printf '%s\n' "NNG4 introductory Lean toolchain pin is unavailable" >&2
    exit 1
  fi
  export ELAN_TOOLCHAIN=$pinned_toolchain
fi

if ! scan_list=$(mktemp "${TMPDIR:-/tmp}/nng4-intro-lean.XXXXXX"); then
  printf '%s\n' "NNG4 introductory Lean source scan failed: could not create a temporary file" >&2
  exit 1
fi
trap 'rm -f "$scan_list"' EXIT HUP INT TERM

if ! find "$project_dir" -type d -name .lake -prune -o -type f -name '*.lean' -print > "$scan_list"; then
  printf '%s\n' "NNG4 introductory Lean source scan failed" >&2
  exit 1
fi
...
if [ "$proof_holes_found" = true ]; then
  printf '%s\n' "NNG4 introductory Lean source contains forbidden proof-placeholder text." >&2
  exit 1
fi
