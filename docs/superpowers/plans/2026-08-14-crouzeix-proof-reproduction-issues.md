...
## Issue 4: DGM-Selected Dedicated Expert Frontier

**Goal:** add the approved archive search over pristine dedicated expert
sessions without changing historical or flat-baseline run semantics.
...
- `labs/crouzeix_proof_reproduction/prompts/expert_*.md`
- `labs/crouzeix_proof_reproduction/prompts/proof_progress_evaluator.md`
- `labs/crouzeix_proof_reproduction/schemas/expert_node.schema.json`
- `labs/crouzeix_proof_reproduction/schemas/proof_progress_evaluation.schema.json`
- `labs/crouzeix_proof_reproduction/tests/test_frontier.py`
- `labs/crouzeix_proof_reproduction/tests/test_expert_runner.py`
- updates to preparation, protocol types, README, and payload receipt

**Test-first slices:**

1. Expert contexts contain exact theorem, role, node/parent/direction identity,
   allowed artifact digests, forbidden sources, completion criteria, and
   `delegation_allowed=false`.
2. Initial contexts contain no peer artifact; child contexts contain only the
   selected parent and direction.
3. Five root experts use the fixed role roster.
4. Two evaluator records must cover exactly ten probes and bind the same node
   digest.
5. Score is unanimous-pass count divided by ten; disagreement does not pass.
6. Functioning-node admission is independent of score improvement.
7. `keep_all` retains lower-scoring functioning children.
8. Eligibility excludes `alpha=1`.
9. Selector computes DGM sigmoid score times inverse functioning-child count.
10. SHA-256 uniform draws, cumulative intervals, and two draws with replacement
    are byte-stable.
11. Only functioning admitted children increment parent child count.
12. Frontier direction ranking follows the approved severity order and stable
    ID tie-break.
13. Attempt, frontier, and selection logs are append-only; snapshot is
    deterministic.
14. Call, node, generation, resource, and archive ceilings fail closed.
15. Fake-provider end-to-end run preserves every failed and lower-scoring node.

**Exact verification:**

```sh
python3 -m unittest \
  labs.crouzeix_proof_reproduction.tests.test_frontier \
  labs.crouzeix_proof_reproduction.tests.test_expert_runner -v
```

**Commit group:** `feat: add DGM-selected Crouzeix expert frontier`

## Issue 5: Run Sealing And Offline Evidence Verification
...
## Issue 6: Live Blind Runs

**Goal:** preserve the historical and flat-baseline attempts, then execute and
seal one DGM-selected dedicated-expert arm under the fixed current runtime.
...
1. Preserve separate `H` and `O0` run directories and interventions.
2. Prepare a fresh `E` run directory.
3. Inspect `run_spec.json`, selector constants, role roster, and seed.
4. Execute `E`; retain every expert, evaluator, selection, and failed call.
5. Seal generation outputs before reference-aware review.
6. Record call, archive, selection, frontier, and aggregate usage accounting.
7. Do not rerun an arm under the same run ID.

**Acceptance:**

- All three attempts have complete receipts or typed failure receipts.
...
- The expert-frontier run proves initial context independence and descendant
  parent scoping through context manifests and parent inventories.
- Every parent draw recomputes from the recorded DGM terms and SHA-256 uniform.
- Every score recomputes from two persisted ten-probe evaluator records.
...
**Commit group:** included with Issue 5 evidence after validation.

## Issue 7: Adversarial Mathematical Review
...
## Issue 8: Packet, Atlas, Search, And Product Contracts
...
## Issue 9: Independent Review And Landing
