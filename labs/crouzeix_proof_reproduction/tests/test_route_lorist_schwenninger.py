...
class LoristSchwenningerSourceLocatorTests(unittest.TestCase):
    ...
    def test_source_locator_rejects_malformed_arxiv_locator_shapes(self) -> None:
        cases = (
            ...
            "arxiv:2608.03841v1:CrouzeixConjecture.pdf#L67-L99",
...
class LoristSchwenningerSchemaParityTests(unittest.TestCase):
    ...
    def test_schema_rejects_each_malformed_arxiv_shape(self) -> None:
        cases = (
            ...
            ("nodes.0.source_locator", "arxiv:2608.03841v1:CrouzeixConjecture.pdf#L67-L99"),
...
class LoristSchwenningerArtifactManifestTests(unittest.TestCase):
    ...
    def test_artifact_manifest_mutations_fail_closed(self) -> None:
        ...
        manifest = json.loads(ARTIFACT_MANIFEST_PATH.read_text(encoding="utf-8"))
        expectations = (
            ("source_identity", "arxiv:2608.03841v2", SOURCE_IDENTITY),
            ("source_id", "LS-ARXIV-V2", SOURCE_ID),
            ("manuscript_path", "Wrong.tex", SOURCE_PATH),
            ("manuscript_bytes", 0, SOURCE_BYTES),
            ("line_count", 0, SOURCE_LINES),
            ("archive_sha256", "0" * 64, SOURCE_ARCHIVE_SHA256),
            ("manuscript_sha256", "0" * 64, SOURCE_FILE_SHA256),
        )
        for field, mutated, expected in expectations:
            with self.subTest(field=field):
                candidate = copy.deepcopy(manifest)
                candidate[field] = mutated
                self.assertNotEqual(candidate[field], expected)
