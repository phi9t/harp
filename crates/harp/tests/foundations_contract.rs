...
#[test]
fn chapters_01_through_08_publish_exact_cards_with_explicit_proof_boundaries() {
    let coverage = contract("coverage.json");
...
#[test]
fn chapters_01_through_08_have_48_distinct_solutions_not_public_card_aliases() {
    let coverage = contract("coverage.json");
    let exercises = contract("exercises.json");
    ...
    assert_eq!(solution_types.len(), 48);
}

#[test]
fn chapters_09_through_12_remain_pending_in_this_scoped_acceptance() {
    let coverage = contract("coverage.json");
    let exercises = contract("exercises.json");
    ...
    let pending_exercises = foundations_rows(&exercises, "exercises")
        ...
        .filter(|row| row["chapter"].as_u64().unwrap() >= 9)
        .collect::<Vec<_>>();
    assert_eq!(pending_cards.len(), 24);
    assert_eq!(pending_exercises.len(), 24);
...
#[test]
fn chapters_01_through_08_meet_the_labeled_editorial_contract() {
    let coverage = contract("coverage.json");
