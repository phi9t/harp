...
use harp_contracts::{
    ...
    ThreadSpec, TurnHandle, TurnSpec, TurnStatus,
};
use harp_runtime::{
    ActivityHandle, ActivityRuntime, ActivitySpec, CodexRuntime,
    InterruptPurpose as RuntimeInterruptPurpose, RuntimeProvenance,
};
use harp_state::{
...
impl Engine {
    ...
    #[allow(dead_code)]
    pub(crate) async fn consume_turn(
        &mut self,
        state: &mut StateStore,
        artifacts: &ArtifactStore,
        ...
        let mut latest_turn_tokens = state
            ...
            .ok_or_else(|| EngineError::ExecutionReceipt {
                context: "turn attempt disappeared before event collection".to_owned(),
            })?
        ...
        let attempt =
            state
                .get_attempt(&claim.attempt_id)?
                .ok_or_else(|| EngineError::ExecutionReceipt {
                    context: "turn attempt disappeared before wall collection".to_owned(),
                })?;
        let wall_started_at =
            attempt
                .wall_started_at
                .ok_or_else(|| EngineError::ExecutionReceipt {
                    context: "turn attempt has no persisted wall tracking start".to_owned(),
                })?;
        ...
        loop {
            ...
            if remaining_wall == 0 {
                let previous_wall = state
                    .get_attempt(&claim.attempt_id)?
                    .ok_or_else(|| EngineError::ExecutionReceipt {
                        context: "turn attempt disappeared at wall deadline".to_owned(),
                    })?
                ...
                self.interrupt_for_limit(state, runtime, lease, turn, &intent)
                    .await?;
            ...
            let event = match self
                .drive_runtime_future(state, lease, wait, runtime.next_event(turn, wait))
                .await
            {
                Ok(event) => event,
                Err(EngineError::RuntimeOperationTimeout { .. }) => {
                    let previous = state
                        .get_attempt(&claim.attempt_id)?
                        .ok_or_else(|| EngineError::ExecutionReceipt {
                            context: "turn attempt disappeared during wall timeout".to_owned(),
                        })?
                        .observed_wall_seconds;
                    let charged = self
            ...
            let previous_wall = state
                .get_attempt(&claim.attempt_id)?
                .ok_or_else(|| EngineError::ExecutionReceipt {
                    context: "turn attempt disappeared during wall reconciliation".to_owned(),
                })?
                .observed_wall_seconds;
            let observed_at = self.wall_clock.unix_seconds()?;
            let wall = self.reconcile_wall_usage_checked(
            ...
            match event {
                RuntimeEvent::TokenUsage(event) => {
                    if event.usage.total_tokens < latest_turn_tokens {
                        return Err(EngineError::InvalidResult {
                            task_id: claim.task_id.clone(),
                            context: "turn token usage decreased".to_owned(),
                        });
                    }
                    ...
                    let expected_previous_turn_tokens = state
                        .get_attempt(&claim.attempt_id)?
                        .ok_or_else(|| EngineError::ExecutionReceipt {
                            context: "turn attempt disappeared during usage reconciliation"
                                .to_owned(),
                        })?
                        .latest_turn_observed_tokens;
                ...
                RuntimeEvent::TurnCompleted(completed) => {
                    ...
                    let output_schema = pinned_output_schema(state, &claim.attempt_id)?;
    ...
    async fn interrupt_for_limit(
        ...
        let operation = state.prepare_interrupt_operation(lease, 0, intent, self.tick()?)?;
        match operation.state {
            ...
            other => {
                return Err(EngineError::ExecutionReceipt {
                    context: format!(
                        "wall-limit interrupt operation is unexpectedly {}",
                        other.as_str()
                    ),
                });
            }
    ...
    #[allow(dead_code)]
    pub(crate) async fn publish_and_accept(
        &mut self,
        state: &mut StateStore,
        artifacts: &ArtifactStore,
        claim: &harp_state::TaskClaim,
        ...
        state.mark_reconciling(lease, self.tick()?)?;
        let operation =
            state.prepare_operation(lease, OperationKind::PublishResult, 0, self.tick()?)?;
        if operation.state == OperationState::Prepared {
            state.mark_operation_dispatching(lease, &operation.operation_id, self.tick()?)?;
        } else if operation.state != OperationState::Dispatching {
            return Err(EngineError::ExecutionReceipt {
                context: format!(
                    "result publication operation is unexpectedly {}",
        ...
        let operation = state
            ...
            .ok_or_else(|| EngineError::ExecutionReceipt {
                context: "result publication operation disappeared".to_owned(),
            })?;
