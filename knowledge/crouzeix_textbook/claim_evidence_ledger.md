...
- Class: `EVIDENCE`
- Statement: The version-two contract has 216 theorem rows: 63 are
  `proved-here`, 115 are `reexported-proof`, 32 are `checkpoint`, and six are
  `definition`. A fresh 459-row Lean receipt checks the declarations required
  by that contract under Lean 4.32.1. The contract has 132 distinct exercise
  solutions; 84 exercises remain correspondence-incomplete. The theorem
  correspondence axis records 132 exact rows, 32 checkpoints, and 52 unmapped
  rows.
