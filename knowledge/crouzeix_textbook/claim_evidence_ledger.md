...
- Statement: The version-two contract has 216 theorem rows: 69 are
  `proved-here`, 117 are `reexported-proof`, 14 are `checkpoint`, and 16 are
  `definition`. A fresh 513-row Lean receipt checks the declarations required
  by that contract under Lean 4.32.1. The contract has 174 distinct exercise
  solutions; 42 exercises remain correspondence-incomplete. The theorem
  correspondence axis records 174 exact rows, 14 checkpoints, and 28 unmapped
  rows.
