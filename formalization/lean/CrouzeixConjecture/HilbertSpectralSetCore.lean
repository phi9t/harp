  ...
  have hzT : z ∈ operatorNumericalRange T := by
    ...
    calc
      inner ℂ xm (T xm) = inner ℂ (xm : H) (A (xm : H)) := by
        exact M.inner_orthogonalProjectionOnto_eq_of_mem_left xm (A (xm : H))
      _ = z := hzx
  have hwT : w ∈ operatorNumericalRange T := by
    refine ⟨ym, hy, ?_⟩
    calc
      inner ℂ ym (T ym) = inner ℂ (ym : H) (A (ym : H)) := by
        exact M.inner_orthogonalProjectionOnto_eq_of_mem_left ym (A (ym : H))
      _ = w := hzy
...
polynomial. -/
theorem norm_operatorPolynomialEval_le_of_forall_closedNumericalRange_of_mainTheorem
  ...
  exact (hilbertSpacePolynomialCrouzeix_of_mainTheorem hMain A p).trans (by gcongr)
...
theorem cauchySeq_operatorPolynomialEval_of_tendstoUniformlyOn_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
  ...
  rw [hevalSub]
  have hbound := norm_operatorPolynomialEval_le_of_forall_closedNumericalRange_of_mainTheorem
    hMain A (q m - q n) (epsilon / 4) (fun z hz ↦ by
...
theorem operatorRationalApproximants_tendsto_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
    (A : H →L[ℂ] H) (r : RatFunc ℂ)
    (hfree : RationalPoleFreeOn r (closedOperatorNumericalRange A)) :
    Tendsto (fun N ↦ operatorPolynomialEval (operatorRationalApproximants A r N) A)
      atTop (nhds (operatorRationalEval r A)) := by
  have hcauchy := cauchySeq_operatorPolynomialEval_of_tendstoUniformlyOn_of_mainTheorem hMain A
...
theorem tendsto_operatorPolynomialEval_sub_zero_of_same_uniform_limit_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
  ...
  rw [hevalSub]
  have hbound := norm_operatorPolynomialEval_le_of_forall_closedNumericalRange_of_mainTheorem
    hMain A (q n - s n) (epsilon / 4) (fun z hz ↦ by
      have hqz := hqN z hz
      have hsz := hsN z hz
      exact (calc
...
theorem tendsto_operatorPolynomialEval_of_tendstoUniformlyOn_polynomial_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
  ...
  have hdiff :=
    tendsto_operatorPolynomialEval_sub_zero_of_same_uniform_limit_of_mainTheorem hMain A q
...
sequence. -/
theorem tendsto_operatorPolynomialEval_of_rational_approximation_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
    (A : H →L[ℂ] H) (r : RatFunc ℂ)
    (hfree : RationalPoleFreeOn r (closedOperatorNumericalRange A))
    (q : ℕ → Polynomial ℂ)
  ...
  have hdiff :=
    tendsto_operatorPolynomialEval_sub_zero_of_same_uniform_limit_of_mainTheorem hMain A q
      ...
      (operatorRationalApproximants_tendstoUniformlyOn A r hfree)
  have hchosen := operatorRationalApproximants_tendsto_of_mainTheorem hMain A r hfree
...
theorem hilbertSpaceRationalCrouzeix_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
    (A : H →L[ℂ] H) (r : RatFunc ℂ)
    (hfree : RationalPoleFreeOn r (closedOperatorNumericalRange A)) :
  ...
  have hoperator := operatorRationalApproximants_tendsto_of_mainTheorem hMain A r hfree
  apply le_of_tendsto hoperator.norm
  filter_upwards [hscalar (epsilon / 8) (by positivity)] with N hN
  have hbound := norm_operatorPolynomialEval_le_of_forall_closedNumericalRange_of_mainTheorem hMain A
...
theorem spectrum_subset_closedOperatorNumericalRange_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement) (A : H →L[ℂ] H) :
  ...
  have hqOperator :
    ...
    exact (cauchySeq_operatorPolynomialEval_of_tendstoUniformlyOn_of_mainTheorem hMain A q
  ...
  have hproduct : TendstoUniformlyOn
    ...
    have hright : TendstoUniformlyOn
        (fun N z ↦ Polynomial.eval z (q N) * Polynomial.eval z d)
        (fun z ↦ Polynomial.eval z 1) atTop (closedOperatorNumericalRange A) :=
      (hproductUniform.congr_right (fun z hz ↦ by
      simp only [Pi.mul_apply]
      simpa only [Polynomial.eval_one] using inv_mul_cancel₀
        (sub_ne_zero.mpr fun hza : z = a ↦ haRange (hza ▸ hz)))).congr <|
  ...
  have hproductOperator :=
    tendsto_operatorPolynomialEval_of_tendstoUniformlyOn_polynomial_of_mainTheorem hMain A
...
theorem operatorPolynomialEval_denom_isUnit_of_rationalPoleFreeOn_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
    (A : H →L[ℂ] H) (r : RatFunc ℂ)
    (hfree : RationalPoleFreeOn r (closedOperatorNumericalRange A)) :
  ...
  exact (rationalPoleFreeOn_iff r (closedOperatorNumericalRange A)).mp hfree z
    (spectrum_subset_closedOperatorNumericalRange_of_mainTheorem hMain A hz) hzero

/-- The approximation-based value is the standard reduced numerator-times-inverse-denominator
rational functional calculus. -/
theorem operatorRationalEval_eq_num_mul_inverse_denom_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
    (A : H →L[ℂ] H) (r : RatFunc ℂ)
    (hfree : RationalPoleFreeOn r (closedOperatorNumericalRange A)) :
    operatorRationalEval r A =
  ...
  have hrightLimit :
    ...
    convert (operatorRationalApproximants_tendsto_of_mainTheorem hMain A r hfree).mul_const
...
theorem hilbertSpaceRationalSpectralSet_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement) :
    HilbertRationalSpectralSetStatement (H := H) :=
  ⟨spectrum_subset_closedOperatorNumericalRange_of_mainTheorem hMain, fun A r hfree ↦ by
    rw [← operatorRationalEval_eq_num_mul_inverse_denom_of_mainTheorem hMain A r hfree]
    exact hilbertSpaceRationalCrouzeix_of_mainTheorem hMain A r hfree⟩
...
theorem closedOperatorNumericalRange_isTwoSpectralSet_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement) (A : H →L[ℂ] H) :
    ClosedOperatorNumericalRangeIsTwoSpectralSet A :=
  ⟨closedOperatorNumericalRange_isCompact A,
    spectrum_subset_closedOperatorNumericalRange_of_mainTheorem hMain A,
    fun r hfree ↦ by
      rw [← operatorRationalEval_eq_num_mul_inverse_denom_of_mainTheorem hMain A r hfree]
      exact hilbertSpaceRationalCrouzeix_of_mainTheorem hMain A r hfree⟩
...
polynomials. -/
theorem operatorRationalEval_algebraMap_polynomial_of_mainTheorem
    (hMain : FiniteMatrixMainTheoremStatement)
  ...
  have hlim := tendsto_operatorPolynomialEval_of_rational_approximation_of_mainTheorem
    hMain A r hfree (fun _ ↦ p) huniform
  have hconst : Tendsto (fun _ : ℕ ↦ operatorPolynomialEval p A) atTop
      (nhds (operatorPolynomialEval p A)) := tendsto_const_nhds
