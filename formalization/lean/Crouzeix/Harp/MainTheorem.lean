import Crouzeix.Harp.FiniteAtomicL2Dilation
import Crouzeix.Harp.FiniteHorizonPerturbation
...
/-!
The Harp finite-horizon route to the finite-matrix polynomial Crouzeix bound.
-/
...
/-- Finite atomic dilations at every horizon, followed by the fixed-domain
simple-spectrum limit and the canonical outer-domain limit, prove the
finite-matrix polynomial Crouzeix bound with constant two. -/
theorem harpFiniteHorizonMainTheorem : MainTheoremStatement (n := n) := by
  intro A p
  ...
  have hbound (k : ℕ) :
    ...
    have hSimpleBound : ∀ᶠ j in atTop,
      ...
      · let cM : ℂ := ((M : ℂ))⁻¹
        ...
        let Gamma := G.parametricBoundary R c
        have hCauchy :
            HasParametricPolynomialCauchyFormula
              Gamma contourParameterMeasure B := by
          simpa only [Gamma] using
            G.hasParametricPolynomialCauchyFormula_of_simpleDiagonalization
              R c B hDiag hWB
        letI : NeZero contourParameterMeasure := ⟨by
          have hmass :
              (∫ _ : ContourParameter, (1 : ℝ)
                  ∂contourParameterMeasure) = contourPeriod := by
            simpa using
              (integral_contourParameter_eq_intervalIntegral
                (fun _ : ℝ => (1 : ℝ)))
          intro hzeroMeasure
          have hperiodZero : contourPeriod = 0 := by
            rw [← hmass, hzeroMeasure]
            simp
          exact contourPeriod_pos.ne' hperiodZero⟩
        let core := finiteHorizonPolynomialCore
          Gamma contourParameterMeasure B hWB q hq
        let witness : ∀ N, FiniteAtomicL2DilationWitness core N :=
          fun N => finiteAtomicL2DilationWitness
            N Gamma contourParameterMeasure B hWB q hq hCauchy
        let L : ℕ → Type := fun N => (witness N).K
        letI : ∀ N, NormedAddCommGroup (L N) :=
          fun N => (witness N).normedAddCommGroup
        letI : ∀ N, InnerProductSpace ℂ (L N) :=
          fun N => (witness N).innerProductSpace
        letI : ∀ N, CompleteSpace (L N) :=
          fun N => (witness N).completeSpace
        letI : ∀ N, FiniteDimensional ℂ (L N) :=
          fun N => (witness N).finiteDimensional
        have hnormalizedOperator :
            ‖euclideanOperator (polynomialEval q B)‖ ≤ 2 := by
          change ‖core.T‖ ≤ 2
          exact norm_target_le_two_of_finiteHorizonDilationData
            (K := L) core (fun N => (witness N).data)
